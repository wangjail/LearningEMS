...
def get_args():
    ...
    parser.add_argument('--algo_name', default='D3QL', type=str, help="name of algorithm")
...
class DuelingDeepQNetwork(torch.nn.Module):
    def __init__(self, state_dim, hidden_dim, hidden_dim1, action_dim):
        ...
        self.fc_A = torch.nn.Linear(hidden_dim1, action_dim)
        self.fc_V = torch.nn.Linear(hidden_dim1, 1)

    def forward(self, x):
        x = F.relu(self.fc1(x))
        A = self.fc_A(F.relu(self.fc2(x)))
        V = self.fc_V(F.relu(self.fc2(x)))
...
class DuelingDQN:
    def __init__(self, state_dim, hidden_dim, hidden_dim1, action_dim, cfg):
        ...
        self.q_net = DuelingDeepQNetwork(state_dim, hidden_dim, hidden_dim1, self.action_dim).to(device)
        self.target_q_net = DuelingDeepQNetwork(state_dim, hidden_dim, hidden_dim1, self.action_dim).to(device)
...
def main():
    ...
    agent = DuelingDQN(state_dim, hidden_dim, hidden_dim1, action_dim, cfg)
